...
#define FUNI_TYPE(X)                                                           \
    int X(const iap_type *iap, const rap_type *rap, int64 ndim,              \
          ...
          double *par, int64 ijac, double *f, double *dfdu,      \
          double *dfdp)
...
#define BCNI_TYPE(X)                                                           \
    int X(const iap_type *iap, const rap_type *rap, int64 ndim,              \
          double *par, const int64 *icp, int64 nbc,                    \
...
#define ICNI_TYPE(X)                                                           \
    int X(const iap_type *iap, const rap_type *rap, int64 ndim,              \
          double *par, const int64 *icp, int64 nint,                   \
...
#define PVLI_TYPE_BVP(X)                                                       \
    int X(iap_type *iap, rap_type *rap, int64 *icp, double *dtm,         \
          int64 *ndxloc, double *ups, int64 *ndim, double *p0,     \
...
#define STPNT_TYPE_AE(X)                                                       \
    int X(iap_type *iap, rap_type *rap, double *par, int64 *icp,         \
          double *u)
...
#define STPNT_TYPE_BVP(X)                                                      \
    int X(iap_type *iap, rap_type *rap, double *par, int64 *icp,         \
          int64 *ntsrs, int64 *ncolrs, double *rlcur,                  \
          double *rldot, int64 *ndxloc, double *ups,                 \
...
#define FNCS_TYPE_AE(X)                                                        \
    double X(iap_type *iap, rap_type *rap, double *par, int64 *icp,  \
                 logical *chng, FUNI_TYPE((*funi)), int64 *m1aaloc,          \
...
int init(iap_type *iap, rap_type *rap, double *par, int64 *icp,
         double *thl, double **thu_pointer, int64 *iuz,
         double *vuz);
...
int stprae(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           FUNI_TYPE((*funi)), double *rds, int64 *m1aaloc,
...
FNCS_TYPE_AE(fnhbae);
FNCS_TYPE_AE(fnuzae);
int stbif(iap_type *iap, rap_type *rap, double *par, int64 *icp,
          ...
          double *dfdp, double *thl, double *thu);
int swpnt(iap_type *iap, rap_type *rap, double *par, int64 *icp,
          double *rds, int64 m1sbloc, double *stud, double *stu,
...
int sthd(iap_type *iap, rap_type *rap, double *par, int64 *icp,
         double *thl, double *thu);
int headng(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           int64 iunit, int64 *n1, int64 *n2);
int stplae(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           double *rlcur, double *u);
int wrline(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           int64 *icu, int64 *ibr, int64 *ntot, int64 *lab,
           double *vaxis, double *u);
int wrtsp8(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           int64 *lab, double *rlcur, double *u);
int wrjac(iap_type *iap, int64 *n, int64 *m1aaloc, double *aa,
          double *rhs);
int msh(const iap_type *iap, const rap_type *rap, double *tm);
int genwts(const int64 ncol, const int64 n1, double *wt,
           double *wp);
...
int adapt(iap_type *iap, rap_type *rap, int64 *nold, int64 *ncold,
          int64 *nnew, int64 *ncnew, double *tm, double *dtm,
          int64 *ndxloc, double *ups, double *vps);
int interp(iap_type *iap, rap_type *rap, int64 *ndim, int64 *n, int64 *nc,
           double *tm, int64 *ndxloc, double *ups, int64 *n1,
           int64 *nc1, double *tm1, double *ups1, double *tm2,
           int64 *itm1);
...
int intwts(iap_type *iap, rap_type *rap, int64 *n, double *z__,
           double *x, double *wts);
int eqdf(iap_type *iap, rap_type *rap, int64 *ntst, int64 *ndim,
         int64 *ncol, double *dtm, int64 *ndxloc, double *ups,
         double *eqf, int64 *iper);
int eig(iap_type *iap, int64 *ndim, int64 *m1a, double *a,
        doublecomplex *ev, int64 *ier);
int nlvc(int64 n, int64 m, int64 k, double *a, double *u);
int nrmlz(int64 *ndim, double *v);
double pi(double r__);
int ge(int64 n, int64 m1a, double *a, int64 nrhs, int64 ndxloc,
       double *u, int64 m1f, double *f, double *det);
...
double rinpr(iap_type *iap, int64 *ndim1, int64 *ndxloc,
                 double *ups, double *vps, double *dtm,
                 double *thu);
double rnrmsq(iap_type *iap, int64 *ndim1, int64 *ndxloc,
                  double *ups, double *dtm, double *thu);
double rintg(iap_type *iap, int64 *ndxloc, int64 ic, double *ups,
                 double *dtm);
double rnrm2(iap_type *iap, int64 *ndxloc, int64 *ic, double *ups,
                 double *dtm);
double rmxups(iap_type *iap, int64 *ndxloc, int64 *i__,
                  double *ups);
double rmnups(iap_type *iap, int64 *ndxloc, int64 *i__,
                  double *ups);
int scaleb(iap_type *iap, int64 *icp, int64 *ndxloc, double *dvps,
...
int contbv(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           ...
           double *rlold, double *rldot, int64 *ndxloc,
...
int extrbv(iap_type *iap, rap_type *rap, FUNI_TYPE((*funi)), double *rds,
           double *rlcur, double *rlold, double *rldot,
           int64 *ndxloc, double *ups, double *uoldps,
           double *udotps);
...
int rsptbv(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           ...
           double *tm, double *dtm, doublecomplex *ev, int64 *nodir,
           double *thl, double *thu);
STPNT_TYPE_BVP(stpnbv);
STPNT_TYPE_BVP(stpnub);
int setrtn(iap_type *iap, int64 *ntst, int64 *ndxloc, double *ups,
           double *par);
int stdrbv(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           FUNI_TYPE((*funi)), BCNI_TYPE((*bcni)), ICNI_TYPE((*icni)),
           double *rlcur, double *rlold, double *rldot,
           int64 ndxloc, double *ups, double *dups,
           double *uoldps, double *udotps, double *upoldp,
           double *fa, double *fc, double *dtm, int64 iperp,
...
int tpspbv(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           doublecomplex *ev);
int stplbv(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           double *rldot, int64 *ndxloc, double *ups,
           double *udotps, double *tm, double *dtm, double *thl,
           double *thu);
int wrtbv8(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           double *rldot, int64 *ndxloc, double *ups,
           double *udotps, double *tm, double *dtm);
int wrtbv9(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           double *rlcur, int64 *ndxloc, double *ups, double *tm,
...
/* autlib2.c */
int solvbv(int64 *ifst, iap_type *iap, rap_type *rap, double *par,
           int64 *icp, FUNI_TYPE((*funi)), BCNI_TYPE((*bcni)),
           ICNI_TYPE((*icni)), double *rds, int64 *nllv,
           double *rlcur, double *rlold, double *rldot,
           int64 *ndxloc, double *ups, double *dups,
           double *uoldps, double *udotps, double *upoldp,
           double *dtm, double *fa, double *fc, double *p0,
           double *p1, double *thl, double *thu);
int setfcdd(int64 *ifst, double *dd, double *fc, int64 *ncb,
            int64 *nrc);
int faft(double *ff, double *fa, int64 *ntst, int64 *nrow,
         int64 *ndxloc);
...
int setrhs(int64 *ndim, int64 *ips, int64 *na, int64 *ntst, int64 *np,
           int64 *ncol, int64 *nbc, int64 *nint, int64 *ncb,
           int64 *nrc, int64 *nra, int64 *nca, int64 *iam, int64 *kwt,
           logical *ipar, FUNI_TYPE((*funi)), BCNI_TYPE((*bcni)),
           ICNI_TYPE((*icni)), int64 *ndxloc, iap_type *iap, rap_type *rap,
           double *par, int64 *icp, double *rds, double *fa,
...
int brbd(double *a, double *b, double *c, double *d,
         ...
         int64 *ifst, int64 *idb, int64 *nllv, double *det,
         int64 *nov, int64 *na, int64 *nbc, int64 *nra, int64 *nca,
         ...
         int64 *icf11, int64 *ipr, int64 *icf1, int64 *icf2,
         int64 *irf, int64 *icf);
int setzero(double *fa, double *fc, int64 *na, int64 *nra,
            int64 *nrc);
int conrhs(int64 *nov, int64 *na, int64 *nra, int64 *nca, double *a,
           int64 *nbc, int64 *nrc, double *c, double *fa,
           double *fc, int64 *irf, int64 *icf, int64 *iam);
int copycp(int64 *iam, int64 *kwt, int64 *na, int64 *nov, int64 *nra,
           int64 *nca, double *a, int64 *ncb, double *b,
           int64 *nrc, double *c, double *a1, double *a2,
           double *bb, double *cc, int64 *irf);
int cpyrhs(int64 *na, int64 *nov, int64 *nra, double *faa,
           double *fa, int64 *irf);
int reduce(int64 *iam, int64 *kwt, logical *par, double *a1,
           double *a2, double *bb, double *cc, double *dd,
           int64 *na, int64 *nov, int64 *ncb, int64 *nrc,
           double *s1, double *s2, double *ca1, int64 *icf1,
           int64 *icf2, int64 *icf11, int64 *ipr, int64 *nbc);
int redrhs(int64 *iam, int64 *kwt, logical *par, double *a1,
           double *a2, double *cc, double *faa, double *fc,
           int64 *na, int64 *nov, int64 *ncb, int64 *nrc,
           double *ca1, int64 *icf1, int64 *icf2, int64 *icf11,
           int64 *ipr, int64 *nbc);
int dimrge(int64 *iam, int64 *kwt, logical *par, double *e,
           double *cc, double *d, double *fc, int64 *ifst,
           int64 *na, int64 *nrc, int64 *nov, int64 *ncb, int64 *idb,
           int64 *nllv, double *fcc, double *p0, double *p1,
           double *det, double *s, double *a2, double *faa,
           double *bb);
int bcksub(int64 *iam, int64 *kwt, logical *par, double *s1,
           double *s2, double *a2, double *bb, double *faa,
           double *fc, double *fcc, double *sol1, double *sol2,
           double *sol3, int64 *na, int64 *nov, int64 *ncb,
           int64 *icf2);
int infpar(int64 *iam, logical *par, double *a, double *b,
           double *fa, double *sol1, double *sol2, double *fc,
           int64 *na, int64 *nov, int64 *nra, int64 *nca, int64 *ncb,
           int64 *irf, int64 *icf);
int rd0(int64 *iam, int64 *kwt, double *d, int64 *nrc);
int print1(int64 *nov, int64 *na, int64 *nra, int64 *nca, int64 *ncb,
           int64 *nrc, double *a, double *b, double *c,
...
/* autlib3.c */
FUNI_TYPE(fnlp);
int fflp(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, double *f, int64 ndm, double *dfdu,
         double *dfdp);
STPNT_TYPE_AE(stpnlp);
FUNI_TYPE(fnc1);
STPNT_TYPE_AE(stpnc1);
FUNI_TYPE(fnc2);
int ffc2(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, double *f, int64 ndm, double *dfdu,
         double *dfdp);
...
FUNI_TYPE(fnti);
FUNI_TYPE(fnhd);
int ffhd(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, double *f, int64 ndm, double *dfdu,
         double *dfdp);
STPNT_TYPE_AE(stpnhd);
FUNI_TYPE(fnhb);
int ffhb(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, double *f, int64 ndm, double *dfdu,
         double *dfdp);
STPNT_TYPE_AE(stpnhb);
FUNI_TYPE(fnhw);
int ffhw(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, double *f, int64 ndm, double *dfdu,
         double *dfdp);
...
BCNI_TYPE(bcps);
ICNI_TYPE(icps);
int pdble(const iap_type *iap, const rap_type *rap, int64 *ndim,
          int64 *ntst, int64 *ncol, int64 *ndxloc, double *ups,
          double *udotps, double *tm, double *par);
STPNT_TYPE_BVP(stpnps);
FUNI_TYPE(fnws);
int ffws(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, int64 ijac, double *f, double *dfdu,
         double *dfdp, int64 ndm, double *dfu, double *dfp);
FUNI_TYPE(fnwp);
int stpnwp(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           int64 *ntsr, int64 *ncolrs, double *rlcur, double *rldot,
           int64 *ndxloc, double *ups, double *udotps,
           double *upoldp, double *tm, double *dtm, int64 *nodir,
           double *thl, double *thu);
FUNI_TYPE(fnsp);
int ffsp(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, int64 ijac, double *f, double *dfdu,
         double *dfdp, int64 ndm, double *dfu, double *dfp);
FUNI_TYPE(fnpe);
int ffpe(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, int64 ijac, double *f, double *dfdu,
         double *dfdp, int64 ndm, double *dfu, double *dfp);
ICNI_TYPE(icpe);
FUNI_TYPE(fnpl);
int ffpl(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, double *f, int64 ndm, double *dfdu,
         double *dfdp);
BCNI_TYPE(bcpl);
ICNI_TYPE(icpl);
int stpnpl(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           int64 *ntsr, int64 *ncolrs, double *rlcur, double *rldot,
           int64 *ndxloc, double *ups, double *udotps,
           double *upoldp, double *tm, double *dtm, int64 *nodir,
           double *thl, double *thu);
FUNI_TYPE(fnpd);
int ffpd(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, double *f, int64 ndm, double *dfdu,
         double *dfdp);
BCNI_TYPE(bcpd);
ICNI_TYPE(icpd);
int stpnpd(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           int64 *ntsr, int64 *ncolrs, double *rlcur, double *rldot,
           int64 *ndxloc, double *ups, double *udotps,
           double *upoldp, double *tm, double *dtm, int64 *nodir,
           double *thl, double *thu);
FUNI_TYPE(fntr);
int fftr(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, double *f, int64 ndm, double *dfdu,
         double *dfdp);
int bctr(const iap_type *iap, const rap_type *rap, int64 ndim,
         double *par, const int64 *icp, int64 nbc, const double *u0,
         const double *u1, double *f, int64 ijac, double *dbc);
ICNI_TYPE(ictr);
int stpntr(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           int64 *ntsr, int64 *ncolrs, double *rlcur, double *rldot,
           int64 *ndxloc, double *ups, double *udotps,
           double *upoldp, double *tm, double *dtm, int64 *nodir,
           double *thl, double *thu);
FUNI_TYPE(fnpo);
int ffpo(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const double *upold,
         const int64 *icp, double *par, double *f, int64 ndm,
         double *dfdu, double *dfdp);
BCNI_TYPE(bcpo);
ICNI_TYPE(icpo);
int fipo(const iap_type *iap, const rap_type *rap, int64 ndim,
         double *par, const int64 *icp, int64 nint, int64 nnt0,
         const double *u, const double *uold, const double *udot,
         const double *upold, double *fi, double *dint,
         int64 ndmt, double *dfdu, double *dfdp);
STPNT_TYPE_BVP(stpnpo);
FUNI_TYPE(fnbl);
int ffbl(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, double *f, int64 ndm, double *dfdu,
         double *dfdp);
BCNI_TYPE(bcbl);
int fbbl(const iap_type *iap, const rap_type *rap, int64 ndim,
         double *par, const int64 *icp, int64 nbc, int64 nbc0,
         const double *u0, const double *u1, double *f,
         double *dbc);
ICNI_TYPE(icbl);
int fibl(const iap_type *iap, const rap_type *rap, int64 ndim,
         double *par, const int64 *icp, int64 nint, int64 nnt0,
         const double *u, const double *uold, const double *udot,
...
BCNI_TYPE(bcni);
ICNI_TYPE(icni);
int fopi(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const int64 *icp, double *par, int64 ijac,
         double *f, double *dfdu, double *dfdp);
/* autlib4.c */
int flowkm(int64 *ndim, double *c0, double *c1, int64 *iid,
           double *rwork, doublecomplex *ev);
int dhhpr(int64 *k, int64 *j, int64 *n, double *x, int64 *incx,
          double *beta, double *v);
int dhhap(int64 *k, int64 *j, int64 *n, int64 *q, double *beta,
          double *v, int64 *job, double *a, int64 *lda);
/* autlib5.c */
FUNI_TYPE(fnho);
int ffho(const iap_type *iap, const rap_type *rap, int64 ndim,
         const double *u, const double *uold, const int64 *icp,
         double *par, double *f, int64 ndm, double *dfdu,
         double *dfdp);
BCNI_TYPE(bcho);
int fbho(const iap_type *iap, const rap_type *rap, int64 ndim,
         double *par, const int64 *icp, int64 nbc, int64 nbc0,
         const double *u0, const double *u1, double *fb,
         double *dbc);
ICNI_TYPE(icho);
int fiho(const iap_type *iap, const rap_type *rap, int64 ndim,
         double *par, const int64 *icp, int64 nint, int64 nnt0,
         const double *u, const double *uold, const double *udot,
         const double *upold, double *fi, double *dint);
int inho(iap_type *iap, int64 *icp, double *par);
int preho(int64 *ndx, int64 *ntsr, int64 *nar, int64 *ndim,
          int64 *ncolrs, double *ups, double *udotps, double *tm,
          double *par);
int stpnho(iap_type *iap, rap_type *rap, double *par, int64 *icp,
           int64 *ntsr, int64 *ncolrs, double *rlcur, double *rldot,
           int64 *ndxloc, double *ups, double *udotps,
           double *upoldp, double *tm, double *dtm, int64 *nodir,
           double *thl, double *thu);
int stpho(iap_type *iap, int64 *icp, double *u, double *par,
          double *t);
...
int eighi(int64 isign, int64 itrans, double *rr, double *ri,
          double *vret, double *xequib, const int64 *icp,
          double *par, int64 *ndm);
int eigho(int64 *isign, int64 *itrans, double *rr, double *ri,
          double *vret, double *xequib, const int64 *icp,
          double *par, int64 *ndm, double *dfdu, double *dfdp,
          double *zz);
int prjcti(double *bound, double *xequib, const int64 *icp,
           double *par, int64 imfd, int64 is, int64 itrans,
           int64 *ndm);
int prjctn(double *bound, double *xequib, const int64 *icp,
           double *par, int64 *imfd, int64 *is, int64 *itrans,
           int64 *ndm, double *dfdu, double *dfdp);
/* eispack.c */
int rg(int64 nm, int64 n, double *a, double *wr, double *wi,
       int64 matz, double *z__, int64 *iv1, double *fv1,
       int64 *ierr);
int hqr(int64 *nm, int64 *n, int64 *low, int64 *igh, double *h__,
        double *wr, double *wi, int64 *ierr);
int hqr2(int64 *nm, int64 *n, int64 *low, int64 *igh, double *h__,
         double *wr, double *wi, double *z__, int64 *ierr);
int cdiv(double *ar, double *ai, double *br, double *bi,
         double *cr, double *ci);
int balanc(int64 *nm, int64 *n, double *a, int64 *low, int64 *igh,
           double *scale);
int balbak(int64 *nm, int64 *n, int64 *low, int64 *igh,
           double *scale, int64 *m, double *z__);
int elmhes(int64 *nm, int64 *n, int64 *low, int64 *igh, double *a,
           int64 *int__);
int eltran(int64 *nm, int64 *n, int64 *low, int64 *igh, double *a,
           int64 *int__, double *z__);
int qzhes(int64 nm, int64 n, double *a, double *b, logical matz,
          double *z__);
int qzit(int64 nm, int64 n, double *a, double *b, double eps1,
         logical matz, double *z__, int64 *ierr);
int qzval(int64 nm, int64 n, double *a, double *b, double *alfr,
...
int daxpy(int64 *n, double *da, double *dx, int64 *incx,
          double *dy, int64 *incy);
int drot(int64 *n, double *dx, int64 *incx, double *dy,
         int64 *incy, double *c, double *s);
int dswap(int64 *n, double *dx, int64 *incx, double *dy,
          int64 *incy);
int dgemc(int64 *m, int64 *n, double *a, int64 *lda, double *b,
          int64 *ldb, logical *trans);
...
int dgemm(char *transa, char *transb, int64 *m, int64 *n, int64 *k,
          double *alpha, double *a, int64 *lda, double *b,
          int64 *ldb, double *beta, double *c, int64 *ldc,
          int64 transa_len, int64 transb_len);
int ezsvd(double *x, int64 *ldx, int64 *n, int64 *p, double *s,
          double *e, double *u, int64 *ldu, double *v,
          int64 *ldv, double *work, int64 *job, int64 *info,
          double *tol);
int ndrotg(double *f, double *g, double *cs, double *sn);
int ndsvd(double *x, int64 *ldx, int64 *n, int64 *p, double *s,
          double *e, double *u, int64 *ldu, double *v,
          int64 *ldv, double *work, int64 *job, int64 *info,
          int64 *maxitr, double *tol, int64 *idbg, int64 *ifull,
          int64 *kount, int64 *kount1, int64 *kount2, int64 *skip,
          int64 *limshf, double *maxsin, int64 *iidir);
int prse(int64 *ll, int64 *m, int64 *nrow, int64 *ncol, double *s,
         double *e);
...
int hqr3lc(double *a, double *v, int64 *n, int64 *nlow,
           int64 *nup, double *eps, double *er, double *ei,
           int64 *type__, int64 *na, int64 *nv, int64 *imfd);
int split(double *a, double *v, int64 *n, int64 *l, double *e1,
          double *e2, int64 *na, int64 *nv);
int exchng(double *a, double *v, int64 *n, int64 *l, int64 *b1,
           int64 *b2, double *eps, logical *fail, int64 *na,
           int64 *nv);
int qrstep(double *a, double *v, double *p, double *q,
           double *r__, int64 *nl, int64 *nu, int64 *n, int64 *na,
           int64 *nv);
int orthes(int64 *nm, int64 *n, int64 *low, int64 *igh, double *a,
           double *ort);
int ortran(int64 *nm, int64 *n, int64 *low, int64 *igh, double *a,
           double *ort, double *z__);

/* problem defined functions*/
int func(int64 ndim, const double *u, const int64 *icp,
         const double *par, int64 ijac, double *f, double *dfdu,
         double *dfdp);
int stpnt(int64 ndim, double t, double *u, double *par);
int bcnd(int64 ndim, const double *par, const int64 *icp, int64 nbc,
         const double *u0, const double *u1, int64 ijac,
         double *f, double *dbc);
int icnd(int64 ndim, const double *par, const int64 *icp, int64 nint,
         const double *u, const double *uold, const double *udot,
         const double *upold, int64 ijac, double *fi,
         double *dint);
int fopt(int64 ndim, const double *u, const int64 *icp,
         const double *par, int64 ijac, double *fs, double *dfdu,
         double *dfdp);
