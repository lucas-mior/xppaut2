...
char *com;
{
  ...
  if (j < NCMD) {
    ...
    if (com[0] == '-' || got_file == 1) {
      ...
      plintf(" -ncdraw 1|2               Draw nullclines in batch (1) to file "
             "(2) \n");
      plintf(
          " -dfdraw 1-5       Draw dfields in batch (1-3) to file (4-5)  \n");
      plintf("  -version               Print XPPAUT version and exit \n");
      plintf("  -readset <filename>   Read in a set file like the internal "
             "sets\n");
      plintf("  -with string   String must be surrounded with quotes; anything "
