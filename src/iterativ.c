...
ModifiedGS(Vector *v, double **h, int32 k, int32 p, double *new_vk_norm) {
    ...
    vk_norm = llnlmath_rsqrt(vector_DotProd(v[k], v[k]));
    k_minus_1 = k - 1;
    i0 = MAX(k - p, 0);
    ...
    for (i = i0; i < k; i++) {
        h[i][k_minus_1] = vector_DotProd(v[i], v[k]);
        vector_LinearSum(ONE, v[k], -h[i][k_minus_1], v[i], v[k]);
    }

    /* Compute the norm of the new vector at v[k].  */

    *new_vk_norm = llnlmath_rsqrt(vector_DotProd(v[k], v[k]));
    ...
    for (i = i0; i < k; i++) {
        new_product = vector_DotProd(v[i], v[k]);
        temp = FACTOR*h[i][k_minus_1];
        if ((temp + new_product) == temp)
            continue;
        h[i][k_minus_1] += new_product;
        vector_LinearSum(ONE, v[k], -new_product, v[i], v[k]);
...
ClassicalGS(Vector *v, double **h, int32 k, int32 p, double *new_vk_norm,
    ...
    *new_vk_norm = llnlmath_rsqrt(vector_DotProd(v[k], v[k]));
    ...
    if ((FACTOR*(*new_vk_norm)) < vk_norm) {
        ...
        for (i = i0 + 1; i < k; i++) {
            vector_LinearSum(s[i], v[i], ONE, temp, temp);
            h[i][k_minus_1] += s[i];
        }
        vector_LinearSum(ONE, v[k], -ONE, temp, v[k]);
