...
cvode_malloc(int64 N, RhsFn f, double t0, Vector y0, int32 lmm, int32 iter,
    ...
    if (itol == SS) {
        ...
        neg_abstol = (vector_N_VMin((Vector)abstol) < ZERO);
    }
    if (neg_abstol) {
...
CVode(void *cvode_mem, double tout, Vector yout, double *t, int32 itask) {
    ...
    *t = tn;
    ...
    while (true) {
        ...
        if ((tolsf = uround*vector_N_VWrmsNorm(zn[0], ewt)) > ONE) {
...
cvode_dky(void *cvode_mem, double t, int32 k, Vector dky) {
    ...
    for (j = q; j >= k; j--) {
        ...
        for (i = j; i >= j - k + 1; i--)
            c *= i;
        if (j == q) {
            vector_N_VScale(c, zn[q], dky);
        } else {
            vector_N_VLinearSum(c, zn[j], s, dky, dky);
        }
    }
    if (k == 0)
        return OKAY;
    r = llnlmath_rpower_i(h, -k);
    vector_N_VScale(r, dky, dky);
    return OKAY;
}
...
CVEwtSetSS(CVodeMem cv_mem, double *rtol, double *atol, Vector ycur,
    ...
    rtoli = *rtol;
    atoli = *atol;
    vector_N_VAbs(ycur, tempv);
    vector_N_VScale(rtoli, tempv, tempv);
    vector_N_VAddConst(tempv, atoli, tempv);
    if (vector_N_VMin(tempv) <= ZERO)
        return FALSE;
    vector_N_VInv(tempv, ewtvec);
    return TRUE;
}
...
CVEwtSetSV(CVodeMem cv_mem, double *rtol, Vector atol, Vector ycur,
    ...
    rtoli = *rtol;
    vector_N_VAbs(ycur, tempv);
    vector_N_VLinearSum(rtoli, tempv, ONE, atol, tempv);
    if (vector_N_VMin(tempv) <= ZERO)
        return FALSE;
    vector_N_VInv(tempv, ewtvec);
    return TRUE;
...
cv_upper_bound_h0(CVodeMem cv_mem, double tdist) {
    ...
    temp1 = tempv;
    temp2 = acor;
    vector_N_VAbs(zn[0], temp1);
    vector_N_VAbs(zn[1], temp2);
    if (vectorAtol) {
        vector_N_VLinearSum(HUB_FACTOR, temp1, ONE, (Vector)abstol, temp1);
    } else {
        vector_N_VScale(HUB_FACTOR, temp1, temp1);
        vector_N_VAddConst(temp1, atoli, temp1);
    }
    vector_N_VDiv(temp2, temp1, temp1);
    hub_inv = vector_N_VMaxNorm(temp1);
...
cv_ydd_norm(CVodeMem cv_mem, double hg) {
    ...
    vector_N_VLinearSum(hg, zn[1], ONE, zn[0], y);
    f(N, tn + hg, y, tempv, f_data);
    nfe++;
    vector_N_VLinearSum(ONE, tempv, -ONE, zn[1], tempv);
    vector_N_VScale(ONE / hg, tempv, tempv);

    yddnrm = vector_N_VWrmsNorm(tempv, ewt);
    return yddnrm;
}
...
cv_adjust_adams(CVodeMem cv_mem, int32 deltaq) {
    ...
    for (j = 2; j < q; j++)
        vector_N_VLinearSum(-l[j], zn[q], ONE, zn[j], zn[j]);
    return;
}
...
cv_increase_bdf(CVodeMem cv_mem) {
    ...
    vector_N_VScale(A1, zn[qmax], zn[L]);
    for (j = 2; j <= q; j++) {
        vector_N_VLinearSum(l[j], zn[L], ONE, zn[j], zn[j]);
    }
    return;
}
...
cv_rescale(CVodeMem cv_mem) {
    ...
    factor = eta;
    for (j = 1; j <= q; j++) {
        vector_N_VScale(factor, zn[j], zn[j]);
        factor *= eta;
    }
    h = hscale*eta;
    hscale = h;
...
cv_predict(CVodeMem cv_mem) {
    ...
    for (k = 1; k <= q; k++)
        for (j = q; j >= k; j--)
            vector_N_VLinearSum(ONE, zn[j - 1], ONE, zn[j], zn[j - 1]);
    return;
}
...
cv_nls_functional(CVodeMem cv_mem) {
    ...
    nfe++;
    vector_N_VConst(ZERO, acor);

    /* Loop until convergence; accumulate corrections in acor */

    while (true) {
        /* Correct y directly from the last f value */
        vector_N_VLinearSum(h, tempv, -ONE, zn[1], tempv);
        vector_N_VScale(rl1, tempv, tempv);
        vector_N_VLinearSum(ONE, zn[0], ONE, tempv, y);
        /* Get WRMS norm of current correction to use in convergence test */
        vector_N_VLinearSum(ONE, tempv, -ONE, acor, acor);
        del = vector_N_VWrmsNorm(acor, ewt);
        vector_N_VScale(ONE, tempv, acor);
        ...
        if (dcon <= ONE) {
            acnrm = (m == 0) ? del : vector_N_VWrmsNorm(acor, ewt);
...
cv_newton_iteration(CVodeMem cv_mem) {
    ...
    while (true) {
        ...
        vector_N_VLinearSum(rl1, zn[1], ONE, acor, tempv);
        vector_N_VLinearSum(gamma, ftemp, -ONE, tempv, tempv);
        ...
        del = vector_N_VWrmsNorm(b, ewt);
        vector_N_VLinearSum(ONE, acor, ONE, b, acor);
        vector_N_VLinearSum(ONE, zn[0], ONE, acor, y);
...
cv_complete_step(CVodeMem cv_mem) {
    ...
    for (j = 0; j <= q; j++)
        vector_N_VLinearSum(l[j], acor, ONE, zn[j], zn[j]);
    qwait--;
    if ((qwait == 1) && (q != qmax)) {
        vector_N_VScale(ONE, acor, zn[qmax]);
        saved_tq5 = tq[5];
    }
    return;
}
...
cv_prepare_next_step(CVodeMem cv_mem, double dsm) {
    ...
    if (etamax == ONE) {
        ...
        vector_N_VScale(ONE / tq[2], acor, acor);
        return;
    }

    /* etaq is the ratio of new to old h at the current order */
    etaq = ONE / (llnlmath_rpower_r(BIAS2*dsm, ONE / L) + ADDON);
...
cv_compute_etaqm1(CVodeMem cv_mem) {
    ...
    etaqm1 = ZERO;
    if (q > 1) {
        ddn = vector_N_VWrmsNorm(zn[q], ewt) / tq[1];
...
cv_compute_etaqp1(CVodeMem cv_mem) {
    ...
    if (q != qmax) {
        ...
        vector_N_VLinearSum(-cquot, zn[qmax], ONE, acor, tempv);
        dup = vector_N_VWrmsNorm(tempv, ewt) / tq[3];
...
cv_handle_failure(CVodeMem cv_mem, int32 kflag) {
    ...
    vector_N_VProd(acor, ewt, tempv);
    vector_N_VAbs(tempv, tempv);
