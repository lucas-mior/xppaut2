...
FILE *fptr;
{
    ...
    case 'o':
        ...
        if (NODE >= (IN_VARS + FIX_VAR)) {
            ...
            if ((ode_names[NODE - FIX_VAR + NMarkov] = (char *)malloc(nn)) ==
                NULL) {
                plintf("Out of memory at line %d\n", NLINES);
                exit(0);
            }
...
void take_apart(bob, value, name) char *bob, *name;
double *value;
{
    int k, i, l;
    char number[40];
...
void find_ker(string,
              alt) /* this extracts the integral operators from the string */
    char *string;
int *alt;
{
                ...
                if (done == 1) {
                    ...
                    if (v.type == COMMAND && v.lhs[0] == 'M' &&
                        ...
                        if (jj ==
                            ...
                            markovarrays =
                                (char **)malloc(nstates * sizeof(char *));
                            markovarrays2 =
                                (char **)malloc(nstates * sizeof(char *));

                            for (istates = 0; istates < nstates; istates++) {
                                markovarrays[istates] =
                                    (char *)malloc(MAXEXPLEN);
...
void strpiece(dest, src, i0, ie) int i0, ie;
char *dest, *src;
{
    int i;
    for (i = i0; i <= ie; i++)
...
void add_varinfo(type, lhs, rhs, nargs, args) int type;
char *lhs;
char *rhs;
int nargs;
...
void subsk(big, new, k, flag) char *big, *new;
int k, flag;
{
    ...
    for (i = 0; i < n_comments; i++) {
        ...
        if (comments[i].aflag)
            orig_comments[i].action =
                (char *)malloc(strlen(comments[i].action) + 1);
...
void default_comments() {
    ...
    for (i = 0; i < orig_ncomments; i++) {
        ...
        if (orig_comments[i].aflag) {
            comments[i].action =
                (char *)malloc(strlen(orig_comments[i].action) + 1);
