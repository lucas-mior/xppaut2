...
void ALINE(a, b, c, d) int a, b, c, d;
{ XDrawLine(display, AutoW.canvas, small_gc, (a), (b), (c), (d)); }

void DLINE(a, b, c, d) double a, b, c, d;
{ ALINE(IXVal(a), IYVal(b), IXVal(c), IYVal(d)); }

void ATEXT(a, b, c) int a, b;
char *c;
...
void auto_motion(ev) XEvent ev;
{
  ...
  if (w == AutoW.canvas) {
    ...
    y = Auto.ymin + (double)(Auto.y0 - j + Auto.hgt) * (Auto.ymax - Auto.ymin) /
                        (double)Auto.hgt;
...
void resize_auto_window(XEvent ev) {
  ...
  if (ev.xconfigure.window == AutoW.base) {
    ...
    if (TrueColorFlag > 0) {
      ...
      XMoveResizeWindow(display, AutoW.hint, xloc, yloc + chgt + addhgt + 10,
                        wid, DCURY + 2);
    }
...
void auto_keypress(ev, used) XEvent ev;
int *used;
{
