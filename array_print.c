...
int array_print(filename, xtitle, ytitle, bottom, nacross, ndown, col0, row0,
                nskip, ncskip, maxrow, maxcol, data, zmin, zmax, tlo, thi, type)
...
void ps_replot(z, col0, row0, nskip, ncskip, maxrow, maxcol, nacross, ndown,
               zmin, zmax, type) float **z;
double zmin, zmax;
...
void ps_begin(xlo, ylo, xhi, yhi, sx, sy) double xlo, ylo, xhi, yhi;
float sx, sy;
{
    float x0, y0, x1, y1;
    ps_scale.xmin = xlo;
...
void ps_convert(x, y, xs, ys) float x, y, *xs, *ys;
{
    *xs = (x - ps_scale.xmin) * ps_scale.xscale + ps_scale.xoff;
    *ys = (y - ps_scale.ymin) * ps_scale.yscale + ps_scale.yoff;
}

void ps_col_scale(y0, x0, dy, dx, n, zlo, zhi, type, mx) double y0, x0, dy, dx,
    zlo, zhi;
float mx;
int n, type;
{
...
void ps_boxit(tlo, thi, jlo, jhi, zlo, zhi, sx, sy, sb, type) double tlo, thi,
    jlo, jhi, zlo, zhi;
int type;
char *sx, *sy, *sb;
{
...
void ps_setline(fill, thick) float fill;
int thick;
{
...
void ps_put_char(ch, x, y) char ch;
float *x, *y;
{
    float xp = *x, yp = *y;
    char str[4];
...
void ps_text2(str, xr, yr, icent) char *str;
float xr, yr;
...
void ps_line2(x1r, y1r, x2r, y2r) float x1r, y1r, x2r, y2r;
{
    float x1, y1, x2, y2;
...
void ps_set_text(angle, slant, x_size, y_size) float angle, slant, x_size,
    y_size;
{
...
void ps_rect(x, y, wid, len) float x, y, wid, len;
{
    float x1, y1, x2, y2;
...
void ps_bar(x, y, wid, len, fill, flag) float x, y, wid, len, fill;
int flag;
{
    float x1, y1, x2, y2;
    fprintf(my_plot_file, "%f G\n", fill);
...
void ps_rgb_bar(x, y, wid, len, fill, flag, rgb) float x, y, wid, len, fill;
int flag, rgb;
{
    float x1, y1, x2, y2;
...
void ps_hsb_bar(x, y, wid, len, fill, flag) float x, y, wid, len, fill;
int flag;
{
    float x1, y1, x2, y2;
