...
int itp;
{
  ...
  if (RestartLabel != 0) {
    printf(
        "RestartLabel=%d itp=%d ips=%d nfpar=%d ilp=%d isw=%d isp=%d A2p=%d \n",
...
{
  NAutoUzr = Auto.nper;
  init_auto(NODE, Auto.nfpar, Auto.nbc, Auto.ips, Auto.irs, Auto.ilp, Auto.ntst,
            Auto.isp, Auto.isw, Auto.nmx, Auto.npr, Auto.ds, Auto.dsmin,
            Auto.dsmax, Auto.rl0, Auto.rl1, Auto.a0, Auto.a1, Auto.icp1,
            Auto.icp2, Auto.icp3, Auto.icp4, Auto.icp5, Auto.nper, Auto.epsl,
            Auto.epsu, Auto.epss, Auto.ncol);
}
...
int flg, lab, npar;
{
  ...
  if (icp2 < NAutoPar)
    par2 = par[icp2];
  auto_xy_plot(&x, &y1, &y2, par1, par2, per, uhigh, ulow, ubar,
  ...
  if (flag2 > 0 &&
      Auto.plot !=
          P_P) { /* two parameter and not in two parameter plot, just skip it */
    plot_stab(evr, evi, NODE);
    refreshdisplay();
    return;
  }
...
void auto_start_at_homoclinic() {
  ...
  Auto.nfpar = 2;
  Auto.ilp = 1; /* maybe 1 someday also in extend homo, but for now, no 3 param
                   allowed    */
  Auto.isw = 1;
  Auto.isp = 0;
  Auto.nbc = 0;
