...
void *setubv_make_aa_bb_cc(void *arg) {
  ...
  integer aa_dim1, aa_dim2, bb_dim1, bb_dim2, cc_dim1, cc_dim2, ups_dim1,
    ...
    for (ic = 0; ic < larg->ncol; ++ic) {
      for (k = 0; k < larg->ndim; ++k) {
        ...
        for (l = 0; l < larg->ncol; ++l) {
          ...
          uold[k] +=
              ARRAY2D(wt, l, ic) * ARRAY2D(uoldps, j + larg->loop_offset, l1);
        }
      }
      ...
      /*
          ...
          checked as well.
      */
      (*(larg->funi))(larg->iap, larg->rap, larg->ndim, u, uold, larg->icp, prm,
                      2, f, dfdu, dfdp);
  ...
  if (larg->nint > 0) {
    for (jj = larg->loop_start; jj < larg->loop_end; ++jj) {
      ...
      for (k = 0; k < (larg->ncol + 1); ++k) {
        ...
        (*(larg->icni))(larg->iap, larg->rap, larg->ndim, larg->par, larg->icp,
                        larg->nint, uic, uio, uid, uip, ficd, 2, dicd);
...
int setubv(integer ndim, integer ips, integer na, integer ncol, integer nbc,
...

  {
    ...
    setubv_parallel_arglist_constructor(
        ndim, ips, na, ncol, nbc, nint, ncb, nrc, nra, nca, funi, icni, ndxloc,
        iap, rap, par, icp, aa, bb, cc, dd, fa, fc, ups, uoldps, udotps, upoldp,
        dtm, wp, wt, wi, thu, thl, rldot, bcni, &arglist);
...
void setubv_make_fa(setubv_parallel_arglist larg) {
  ...
  doublereal *wploc =
      (doublereal *)malloc(sizeof(doublereal) * (larg.ncol) * (larg.ncol + 1));
  ...
  for (jj = 0; jj < larg.na; ++jj) {
    ...
    for (ic = 0; ic < larg.ncol; ++ic) {
      ...
      (*(larg.funi))(larg.iap, larg.rap, larg.ndim, u, uold, larg.icp, prm, 2,
                     f, dfdu, dfdp);
...
void setubv_make_fc_dd(setubv_parallel_arglist larg, doublereal *dups,
  ...
  if (larg.nbc > 0) {
    ...
    (*(larg.bcni))(larg.iap, larg.rap, larg.ndim, larg.par, larg.icp, larg.nbc,
                   ubc0, ubc1, fbc, 2, dbc);
    for (i = 0; i < larg.nbc; ++i) {
      larg.fc[i] = -fbc[i];
      for (k = 0; k < larg.ncb; ++k) {
