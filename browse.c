...
BROWSER *b;
{
    ...
    if ((my_ode[NEQ + FIX_VAR] = (int *)malloc((i + 2) * sizeof(int))) ==
        NULL) {
...
void make_d_table(xlo, xhi, col, filename, b) int col;
double xlo, xhi;
char *filename;
BROWSER b;
{
...
void find_value(col, val, row, b) int col, *row;
float val;
BROWSER b;
{
    int n = b.maxrow;
...
void browse_but_on(b, i, w, yn) int i;
Window w;
int yn;
BROWSER *b;
{
