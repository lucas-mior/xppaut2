...
void set_old_arg_names(narg) int narg;
{
  int i;
  for (i = 0; i < narg; i++) {
...
void set_new_arg_names(narg, args) char args[10][11];
int narg;
{
  int i;
  for (i = 0; i < narg; i++) {
...
void fixup_endfun(u, l, narg) int *u;
int l, narg;
{
  u[l - 1] = ENDFUN;
  u[l] = narg;
...
void find_name(string, index) char *string;
int *index;
{
  char junk[100];
  int i, len;
...
void pr_command(command) int *command;
{
  int i = 0;
  int token;
  while (1) {
...
void show_where(string, index) char *string;
int index;
{
  char junk[MAXEXPLEN];
  int i;
...
int function_sym(int token) /* functions should have ( after them  */
{
  ...
  if (token == DELSHFTSYM || token == SETSYM || token == DELSYM ||
      token == SHIFTSYM || token == ISHIFTSYM || com == MYIF || com == MYTHEN ||
      com == MYELSE || com == SUMSYM || com == ENDSUM)
    return (1);
  return (0);
}
...
void convert(source, dest) char *source, *dest;
{
  char ch;
  int i = 0, j = 0;
  while (1) {
...
void find_tok(source, index, tok) char *source;
int *index, *tok;
{
