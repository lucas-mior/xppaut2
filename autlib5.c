...
/* Subroutine */ int fnho(const iap_type *iap, const rap_type *rap,
  ...
  for (i = 0; i < ndim; ++i) {
    ...
    ffho(iap, rap, ndim, global_scratch.uu1, uold, icp, par, global_scratch.ff1,
         ndm, global_scratch.dfu, global_scratch.dfp);
    ffho(iap, rap, ndim, global_scratch.uu2, uold, icp, par, global_scratch.ff2,
         ndm, global_scratch.dfu, global_scratch.dfp);
    for (j = 0; j < ndim; ++j) {
      ARRAY2D(dfdu, j, i) =
...
/* Subroutine */ int pvlsho(iap_type *iap, rap_type *rap, integer *icp,
  ...
  if (blhom_1.itwist == 1) {
    eighi(1, 1, bleig_1.rr, bleig_1.ri, bleig_1.vt, bleig_1.xequib, icp, par,
          &ndm);
    bleig_1.ineig = 1;
    orient =
        psiho(iap, 0, bleig_1.rr, bleig_1.ri, bleig_1.v, bleig_1.vt, icp, par);
    if (iid >= 3) {
      if (orient < 0.) {
