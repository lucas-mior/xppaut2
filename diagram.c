...
int find_diagram(irs, n, index, ibr, ntot, itp, nfpar, a, uhi, ulo, u0, par,
                 per, icp1, icp2, icp3, icp4)
...
void edit_start(ibr, ntot, itp, lab, nfpar, a, uhi, ulo, u0, ubar, par, per, n,
                icp1, icp2, icp3, icp4, evr, evi) int ibr,
    ntot, itp, lab, nfpar, n, icp1, icp2, icp3, icp4;
double *par, per, a;
double *evr, *evi;
...
void edit_diagram(d, ibr, ntot, itp, lab, nfpar, a, uhi, ulo, u0, ubar, par,
                  per, n, icp1, icp2, icp3, icp4, flag2, evr, evi,
                  tp) DIAGRAM *d;
...
void add_diagram(ibr, ntot, itp, lab, nfpar, a, uhi, ulo, u0, ubar, par, per, n,
                 icp1, icp2, icp3, icp4, flag2, evr, evi) int ibr,
    ntot, itp, lab, n, icp1, icp2, icp3, icp4, flag2, nfpar;
double *par, per, a;
double *uhi, *ulo, *u0, *ubar;
double *evr, *evi;
{
...
void redraw_diagram() {
  ...
  while (1) {
    ...
    add_point(d->par, d->per, d->uhi, d->ulo, d->ubar, d->norm, type, flag,
              d->lab, d->nfpar, d->icp1, d->icp2, d->icp3, d->icp4, d->flag2,
              d->evr, d->evi);
    d = d->next;
    if (d == NULL)
      break;
...
void bound_diagram(xlo, xhi, ylo, yhi) double *xlo, *xhi, *ylo, *yhi;
{
  DIAGRAM *d;
  int type;
...
int node;
{
  ...
  while (1) {
    fscanf(fp, "%d %d %d %d %d %d %d %d %d %d %d %d", &calc, &ibr, &ntot, &itp,
           &lab, &index, &nfpar, &icp1, &icp2, &icp3, &icp4, &flag2);
    for (i = 0; i < 8; i++)
      fscanf(fp, "%lg ", &par[i]);
    ...
    if (flag == 0) {
      edit_start(ibr, ntot, itp, lab, nfpar, norm, uhi, ulo, u0, ubar, par, per,
                 node, icp1, icp2, icp3, icp4, evr, evi);
      flag = 1;
      DiagFlag = 1;
    } else
      add_diagram(ibr, ntot, itp, lab, nfpar, norm, uhi, ulo, u0, ubar, par,
                  per, node, icp1, icp2, icp3, icp4, flag2, evr, evi);
    if (index >= n)
      break;
  }
  return (1);
