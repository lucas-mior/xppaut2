...
/* Subroutine */ int qzval(integer nm, integer n, doublereal *a, doublereal *b,
  ...
  static doublereal r__, s, t, a1, a2, u1, u2, v1, v2, a11, a12, a21, a22, b11,
      b12, b22, di, ei;
  static integer na;
...
/* Subroutine */ int dgemc(integer *m, integer *n, doublereal *a, integer *lda,
  ...
  if (*trans) {
    ...
    L80:
      mmp1 = mm + 1;
      i__2 = *m;
      for (i__ = mmp1; i__ <= i__2; i__ += 7) {
        b[j + i__ * b_dim1] = a[i__ + j * a_dim1];
        b[j + (i__ + 1) * b_dim1] = a[i__ + 1 + j * a_dim1];
        b[j + (i__ + 2) * b_dim1] = a[i__ + 2 + j * a_dim1];
        b[j + (i__ + 3) * b_dim1] = a[i__ + 3 + j * a_dim1];
    ...
    L180:
      ...
      for (i__ = mmp1; i__ <= i__2; i__ += 7) {
        ...
        b[i__ + 2 + j * b_dim1] = a[i__ + 2 + j * a_dim1];
        b[i__ + 3 + j * b_dim1] = a[i__ + 3 + j * a_dim1];
...
/* Subroutine */ int ezsvd(doublereal *x, integer *ldx, integer *n, integer *p,
  ...
  ndsvd(&x[x_offset], ldx, n, p, &s[1], &e[1], &u[u_offset], ldu, &v[v_offset],
        ldv, &work[1], job, info, &maxitr, tol, &idbg, &ifull, &kount, &kount1,
        &kount2, &skip, &limshf, &maxsin, &iidir);
  return 0;
} /* ezsvd_ */
  ...
  *info = 0;
  ...
  L300:
    ...
    for (ll = 1; ll <= i__1; ++ll) {
      ...
      for (j = lp1; j <= i__2; ++j) {
        i__3 = *p - l;
        t = -ddot(&i__3, &v[lp1 + l * v_dim1], &c__1, &v[lp1 + j * v_dim1],
                  &c__1) /
            v[lp1 + l * v_dim1];
        i__3 = *p - l;
        daxpy(&i__3, &t, &v[lp1 + l * v_dim1], &c__1, &v[lp1 + j * v_dim1],
              &c__1);
        /* L310: */
      }
    L320:
  ...
  if (*tol > (double)0.) {
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
      /* Computing MAX */
      d__3 = smax, d__4 = (d__1 = s[i__], fabs(d__1)), d__3 = max(d__3, d__4),
      d__4 = (d__2 = e[i__], fabs(d__2));
      smax = max(d__3, d__4);
      /* L1111: */
    }
...
L1003:
  ...
  if (ll == m - 1) {
    ...
    s[m] = sigmn;
    if (wantv) {
      drot(p, &v[ll * v_dim1 + 1], &c__1, &v[m * v_dim1 + 1], &c__1, &cosr,
           &sinr);
    }
    /*       if wantu and ll.eq.n, then rotation trivial */
    if (wantu && ll < *n) {
      drot(n, &u[ll * u_dim1 + 1], &c__1, &u[m * u_dim1 + 1], &c__1, &cosl,
           &sinl);
    }
    goto L999;
  }
    ...
    if (idir == 1) {
      ...
      for (k = ll; k <= i__1; ++k) {
        ...
        *maxsin = max(d__1, d__2);
        ...
        if (wantv) {
          drot(p, &v[k * v_dim1 + 1], &c__1, &v[(k + 1) * v_dim1 + 1], &c__1,
               &cs, &sn);
        }
        if (k != ll) {
          e[k - 1] = oldsn * f;
        ...
        *maxsin = max(d__1, d__2);
        ...
        /*           if wantu and k.eq.n, then s(k+1)=0 so g=0 so cs=1
         and sn=0 */
        if (wantu && k < *n) {
          drot(n, &u[k * u_dim1 + 1], &c__1, &u[(k + 1) * u_dim1 + 1], &c__1,
               &cs, &sn);
        }
        s[k] = f;
      ...
      for (k = m; k >= i__1; --k) {
        ...
        *maxsin = max(d__1, d__2);
        ...
        if (wantu) {
          d__1 = -sn;
          drot(n, &u[(k - 1) * u_dim1 + 1], &c__1, &u[k * u_dim1 + 1], &c__1,
               &cs, &d__1);
        }
        if (k != m) {
          e[k] = oldsn * f;
        ...
        *maxsin = max(d__1, d__2);
        ...
        if (wantv) {
          d__1 = -sn;
          drot(p, &v[(k - 1) * v_dim1 + 1], &c__1, &v[k * v_dim1 + 1], &c__1,
               &cs, &d__1);
        }
        s[k] = f;
        /*           if (idbg.gt.3) print *,'s(k)=',s(k) */
...
L998:
  ...
  for (i__ = 1; i__ <= i__1; ++i__) {
    ...
    if (iisub != m + 1 - i__) {
      ...
      s[iisub] = temp;
      if (wantv) {
        dswap(p, &v[(m + 1 - i__) * v_dim1 + 1], &c__1, &v[iisub * v_dim1 + 1],
              &c__1);
      }
      if (wantu) {
        dswap(n, &u[(m + 1 - i__) * u_dim1 + 1], &c__1, &u[iisub * u_dim1 + 1],
              &c__1);
      }
    }
    /* L1011: */
...
/* Subroutine */ int sig22(doublereal *a, doublereal *b, doublereal *c__,
  ...
  /* Local variables */
  static doublereal absa, absb, absc, acmn, acmx, sgna, sgnb, sgnc, cosl, sinl,
...
int get_qp(double *a1, int n, double *qr, double *qi, double *pr, double *pi) {
  ...
  for (i = 0; i < n; i++) {
    pr[i] = z[n * j + i];
    pi[i] = -z[n * (j + 1) +
               i]; /* get AT z = i w z, take CC to get AT zbar = -i w zbar */
  }
