...
void addbif(iap_type *iap, rap_type *rap, integer ntots, integer ibrs,
  ...
  if (iap->ntot == 1) {
    add_point(par, per, uhigh, ulow, ubar, *a, type, 0, lab, iap->nfpr, icp1,
              icp2, icp3, icp4, AutoTwoParam, my_ev.evr, my_ev.evi);
  } else {
    add_point(par, per, uhigh, ulow, ubar, *a, type, 1, lab, iap->nfpr, icp1,
              icp2, icp3, icp4, AutoTwoParam, my_ev.evr, my_ev.evi);
  }
  ...
  if (DiagFlag == 0) {
    ...
    edit_start(ibrs, ntots, iap->itp, lab, iap->nfpr, *a, uhigh, ulow, u0, ubar,
               par, per, iap->ndim, icp1, icp2, icp3, icp4, my_ev.evr,
               my_ev.evi);
    DiagFlag = 1;
    return;
  }
  add_diagram(ibrs, ntots, iap->itp, lab, iap->nfpr, *a, uhigh, ulow, u0, ubar,
              par, per, iap->ndim, icp1, icp2, icp3, icp4, AutoTwoParam,
              my_ev.evr, my_ev.evi);
}
